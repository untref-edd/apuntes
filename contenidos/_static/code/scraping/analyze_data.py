...
def clean_data(df):
    ...
    df_clean["in_stock"] = df_clean["availability"].str.contains("In stock", case=False, na=False)

    # Extraer número de libros disponibles si está en el texto
    df_clean["stock_number"] = df_clean["availability"].str.extract(r"(\d+)", expand=False).astype("Int64")
...
def rating_analysis(df):
    ...
    if df["price"].notna().any() and valid_ratings.any():
        price_by_rating = df.groupby("rating_numeric")["price"].agg(["count", "mean", "median", "std"]).round(2)

        print(f"\nPrecio promedio por calificación:")
        for rating, stats in price_by_rating.iterrows():
            if pd.notna(rating):
                print(f"  {int(rating)} estrella(s): £{stats['mean']:.2f} " f"(n={stats['count']})")
...
def export_summary(df, output_file="books_summary.txt"):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        if df["price"].notna().any():
            ...
            f.write(f"  Rango de precios: £{df['price'].min():.2f} - " f"£{df['price'].max():.2f}\n\n")

        f.write("TOP 10 LIBROS:\n")
        for i, (_, book) in enumerate(df.head(10).iterrows(), 1):
            f.write(f"  {i:2d}. {book['title']}\n")
            f.write(f"      Precio: £{book['price']:.2f} | " f"Rating: {book['rating']}\n")
...
def main():
    ...
    try:
        ...
        print("Nota: matplotlib/seaborn no disponibles. " "No se generarán gráficos.")

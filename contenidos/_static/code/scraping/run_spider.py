...
def run_spider(
    spider_name="books", output_format="csv", output_file=None, log_level="INFO", delay=1, concurrent_requests=8
):
    """
    ...
    if output_file:
        ...
        default_files = {"csv": "books_output.csv", "json": "books_output.json", "xml": "books_output.xml"}
...
Ejemplos de uso:
  ...
  python run_spider.py --concurrent 4           # 4 requests concurrentes
    ...
    parser.add_argument("--spider", "-s", default="books", help="Nombre del spider a ejecutar (default: books)")

    parser.add_argument(
        "--format", "-f", choices=["csv", "json", "xml"], default="csv", help="Formato de salida (default: csv)"
    )

    parser.add_argument("--output", "-o", help="Archivo de salida personalizado")

    parser.add_argument(
        "--delay", "-d", type=float, default=1.0, help="Delay entre requests en segundos (default: 1.0)"
    )

    parser.add_argument("--concurrent", "-c", type=int, default=8, help="Número de requests concurrentes (default: 8)")

    parser.add_argument("--debug", action="store_true", help="Activar modo debug (logging detallado)")

    parser.add_argument("--check", action="store_true", help="Solo verificar estructura del proyecto")

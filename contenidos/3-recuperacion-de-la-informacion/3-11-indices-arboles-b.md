...
| ![Promoción hacia arriba](../assets/images/bplus4.png)<br>Como "PILA", es la primera clave del segundo nodo que se partió, se promueve hacia arriba. | ![Nueva raíz](../assets/images/bplus5.png)<br>El nodo intermedio también se parte y finalmente se promueve "PILA" a la raíz.                              |
...
<iframe src="https://opendsa-server.cs.vt.edu/embed/bPlusTreeInsertPRO" height="600" width="100%" scrolling="yes"></iframe>
...
```{note}
**Ventajas de usar BTrees:**
- Implementación madura del proyecto ZODB (Zope Object Database)
...
**Características de los árboles B+ en BTrees:**
- Búsqueda: O(log n)
- Inserción: O(log n)
- Eliminación: O(log n)
...
```{note}
**Eficiencia de las búsquedas por rango en árboles B+:**
...
**Importante:** El parámetro `max` es exclusivo, es decir, `keys(min='A', max='B')` devuelve claves >= 'A' y < 'B'. Para incluir el límite superior, se puede usar un valor ligeramente mayor (como 'PICOZ' para incluir 'PICO').
```

### Búsqueda con Comodín "?" (un solo carácter)

Para buscar "P?CO" (donde "?" representa un solo carácter), podemos iterar sobre un rango amplio y filtrar las claves que coincidan con el patrón:
...
# Búsqueda con patrón "P?CO" (donde ? es un carácter cualquiera)
print("Búsqueda con patrón 'P?CO' (un carácter en la segunda posición):")
...
```{note}
**Estrategia para búsquedas con comodines en posiciones intermedias:**
